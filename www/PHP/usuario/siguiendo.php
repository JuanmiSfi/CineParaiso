...
<body>
    ...
    <div class='body'>
        ...
        <div class="contenedor-follow">
            <div class='follow'>
                <?php
                $sql = "SELECT u.usuario,u.fto_perfil,s.id_sigue,s.id_usuario FROM siguen s, usuario u WHERE  u.id = s.id_sigue  AND id_usuario = $idusuario ";
                $consult = mysqli_query($conn, $sql);
                ...
                if ($numerofilas > 0) {
                    for ($i = 0; $i < $numerofilas; $i++) {
                        ...
                        $sql = "SELECT COUNT(*) FROM siguen WHERE id_usuario=$_SESSION[idusuario] AND $idseguidor=id_sigue";
                        $resul = mysqli_query($conn, $sql);
